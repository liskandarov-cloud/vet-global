...
export function SellerContractsPanel() {
  ...
  const create = async () => {
    if (!form.offerId || !form.buyerEmail || form.price <= 0) { toast.error('Заполните оффер, email и цену'); return; }
    try {
      ...
      toast.success('Договорная цена сохранена');
  ...
  const del = async (id: string) => {
    if (!confirm('Удалить договорную цену?')) return;
    try { await api.delete(`/contract-prices/${id}`); load(); } catch { toast.error('Ошибка'); }
  };

  return (
    <div className="mt-6">
      <p className="mb-3 text-sm text-ink-muted">Назначайте персональные цены ключевым покупателям — они увидят их автоматически и в заказе.</p>

      <div className="card mb-4 space-y-3 p-4">
        <div className="flex items-center gap-2 font-medium"><Handshake size={18} className="text-teal-700" /> Новая договорная цена</div>
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs text-ink-muted">Ваш оффер (товар)</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-ink-muted">Email покупателя</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-ink-muted">Договорная цена, сум</label>
            <input className="input" type="number" value={form.price} onChange={(e) => setForm({ ...form, price: Number(e.target.value) })} />
          </div>
          <div>
            <label className="mb-1 block text-xs text-ink-muted">Действует до (необязательно)</label>
            <input className="input" type="date" value={form.validUntil} onChange={(e) => setForm({ ...form, validUntil: e.target.value })} />
          </div>
        </div>
        <button className="btn-primary" onClick={create}><Plus size={16} /> Сохранить</button>
      </div>

      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="text-left text-ink-subtle">
            <tr className="border-b border-slate-100"><th className="py-2">Товар</th><th>Покупатель</th><th>Цена</th><th>До</th><th></th></tr>
          </thead>
          <tbody>
            {contracts.length === 0 && <tr><td colSpan={5} className="py-6 text-center text-ink-subtle">Договорных цен нет</td></tr>}
            {contracts.map((c) => (
              <tr key={c.id} className="border-b border-slate-50">
                ...
                <td className="text-ink-subtle">{c.validUntil ? new Date(c.validUntil).toLocaleDateString('ru-RU') : 'бессрочно'}</td>
