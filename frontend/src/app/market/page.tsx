  ...
  if (ready && !user) {
    return (
      <div className="mx-auto max-w-md px-4 py-24 text-center">
        ...
        <p className="mt-4 text-ink-muted">Аналитика рынка доступна поставщикам платформы.</p>
        <Link href="/login" className="btn-primary mt-6 inline-flex">Войти</Link>
      </div>
    );
  }
  if (ready && !allowed) {
    return <div className="mx-auto max-w-md px-4 py-24 text-center text-ink-muted">Раздел доступен только поставщикам и администраторам.</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10">
      <div className="mb-6">
        ...
        <h1 className="mt-3 section-title">Аналитика рынка</h1>
        <p className="mt-2 text-ink-muted">Спрос, цены и ваша позиция на рынке ветеринарных товаров.</p>
      </div>

      {/* Totals */}
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="card p-5"><div className="text-xs text-ink-subtle">GMV платформы</div><div className="mt-1 font-heading text-2xl font-bold">{formatMoney(ins?.totals.revenue ?? 0)}</div></div>
        <div className="card p-5"><div className="text-xs text-ink-subtle">Продано единиц</div><div className="mt-1 font-heading text-2xl font-bold">{(ins?.totals.units ?? 0).toLocaleString('ru-RU')}</div></div>
        <div className="card p-5"><div className="text-xs text-ink-subtle">Ваша выручка</div><div className="mt-1 font-heading text-2xl font-bold text-teal-700">{formatMoney(pos?.myRevenue ?? 0)}</div></div>
      </div>

      {/* Спрос по категориям */}
      <section className="mt-8">
        <h2 className="mb-3 font-heading text-lg font-bold">Спрос по категориям</h2>
      ...
      {/* Бенчмарк цен */}
      <section className="mt-8">
        <h2 className="mb-3 font-heading text-lg font-bold">Бенчмарк цен по категориям</h2>
        <div className="overflow-x-auto rounded-2xl border border-slate-200">
          <table className="w-full min-w-[560px] text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase text-ink-subtle">
              <tr><th className="px-4 py-3">Категория</th><th className="px-4 py-3">Предложений</th><th className="px-4 py-3">Мин.</th><th className="px-4 py-3">Средняя</th><th className="px-4 py-3">Макс.</th></tr>
            </thead>
            <tbody>
      ...
      {ins?.topProducts && ins.topProducts.length > 0 && (
        <section className="mt-8">
          <h2 className="mb-3 font-heading text-lg font-bold">Топ-товары по выручке</h2>
      ...
      {pos && pos.pricePositioning.length > 0 && (
        <section className="mt-8">
          <h2 className="mb-3 font-heading text-lg font-bold">Ваше ценовое позиционирование</h2>
          <div className="overflow-x-auto rounded-2xl border border-slate-200">
            <table className="w-full min-w-[600px] text-sm">
              <thead className="bg-slate-50 text-left text-xs uppercase text-ink-subtle">
                <tr><th className="px-4 py-3">Товар</th><th className="px-4 py-3">Ваша цена</th><th className="px-4 py-3">Рынок мин.</th><th className="px-4 py-3">Рынок средн.</th><th className="px-4 py-3">Δ к рынку</th></tr>
              </thead>
              <tbody>
                {pos.pricePositioning.map((p) => (
                  <tr key={p.product} className="border-t border-slate-100">
                    <td className="px-4 py-2.5 font-medium">{p.product}{p.isCheapest && <span className="ml-2 rounded bg-teal-600 px-1.5 py-0.5 text-[10px] font-semibold text-white">лучшая</span>}</td>
      ...
      {pos && pos.categoryShare.length > 0 && (
        <section className="mt-8">
          <h2 className="mb-3 font-heading text-lg font-bold">Ваша доля в категориях</h2>
          <div className="space-y-2">
            {pos.categoryShare.map((c) => (
              <div key={c.category} className="card p-4">
                <div className="mb-1 flex items-center justify-between text-sm">
                  <span className="font-medium">{c.category}</span>
                  <span className="text-ink-subtle">{formatMoney(c.myRevenue)} из {formatMoney(c.marketRevenue)} · <b className="text-teal-700">{c.sharePct}%</b></span>
                </div>
